...
function hero(input: HeroInput): HeroDef {
  ...
  return {
    ...
    roles: input.roles,
    region: input.region,
    lore: `${input.name} keeps the signature Dota decision while scaling the execution down to one controlled hero and AI-driven extras.`,
...
export const COMPLEX_MISSING_HEROES: HeroDef[] = [
  hero({
    ...
    abilities: [
      ...
      { id: 'ck-phantasm', name: 'Phantasm', targeting: 'no-target', ult: true, castPoint: 0.35, manaCost: [125, 200, 275], cooldown: [95, 85, 75], values: { count: [2, 3, 4], duration: [22, 24, 26], damage: [40, 60, 80] }, effects: [{ kind: 'summon', summon: chaosIllusion, count: 'count', at: 'self' }, { kind: 'statmod', mods: { damage: 'damage' }, duration: 'duration', target: 'self' }], vfx: vfx('summon-pop', '#d84a32', '#f0c060') }
    ]
  }),
  hero({
    ...
    abilities: [
      ...
      { id: 'arc-tempest-double', name: 'Tempest Double', targeting: 'no-target', ult: true, castPoint: 0.35, manaCost: [0, 0, 0], cooldown: [70, 60, 50], values: { duration: [16, 20, 24], damage: [25, 45, 65] }, effects: [{ kind: 'exotic', id: 'tempest-double', params: { heroId: 'arc-warden' } }, { kind: 'summon', summon: arcDouble, count: 1, at: 'self' }, { kind: 'statmod', mods: { damage: 'damage' }, duration: 'duration', target: 'self' }], vfx: vfx('summon-pop', '#65d8ff', '#f6f0a8') }
    ]
  }),
  hero({
    ...
    abilities: [
      ...
      { id: 'meepo-poof', name: 'Poof', targeting: 'point-target', castRange: 700, castPoint: 0.6, manaCost: [80, 80, 80, 80], cooldown: [12, 10, 8, 6], values: { damage: [70, 110, 150, 190], radius: [300, 325, 350, 375], distance: [450, 525, 600, 675] }, effects: [{ kind: 'damage', dtype: 'magical', amount: 'damage', target: 'enemies-in-radius', radius: 'radius' }, { kind: 'displace', mode: 'blink', target: 'self', toward: 'point', distance: 'distance' }, { kind: 'damage', dtype: 'magical', amount: 'damage', target: 'enemies-in-radius', radius: 'radius' }], vfx: vfx('global-mark', '#d8f0a0') },
  ...
  hero({
    ...
    abilities: [
      { id: 'morph-waveform', name: 'Waveform', targeting: 'point-target', castRange: 900, castPoint: 0, manaCost: [115, 125, 135, 145], cooldown: [21, 18, 15, 12], values: { distance: [500, 625, 750, 875], damage: [90, 150, 210, 270], radius: [250, 275, 300, 325] }, effects: [{ kind: 'displace', mode: 'blink', target: 'self', toward: 'point', distance: 'distance' }, { kind: 'damage', dtype: 'magical', amount: 'damage', target: 'enemies-in-radius', radius: 'radius' }], vfx: vfx('global-mark', '#5ad8ff') },
  ...
  hero({
    ...
    abilities: [
      ...
      { id: 'pl-juxtapose', name: 'Juxtapose', targeting: 'no-target', ult: true, castPoint: 0.2, manaCost: [75, 100, 125], cooldown: [85, 70, 55], values: { count: [3, 4, 5], duration: [18, 22, 26], damage: [20, 35, 50] }, effects: [{ kind: 'summon', summon: lancerIllusion, count: 'count', at: 'self' }, { kind: 'statmod', mods: { damage: 'damage' }, duration: 'duration', target: 'self' }], vfx: vfx('summon-pop', '#4a8cff', '#d8e8ff') }
    ]
  }),
  hero({
    ...
    abilities: [
      ...
      { id: 'rubick-spell-steal', name: 'Spell Steal', targeting: 'unit-target', affects: 'enemy', ult: true, castRange: 900, castPoint: 0.15, manaCost: [25, 25, 25], cooldown: [70, 55, 40], values: { duration: [14, 18, 22], damage: [160, 240, 320] }, effects: [{ kind: 'exotic', id: 'spell-steal', params: { heroId: 'rubick' } }, { kind: 'damage', dtype: 'magical', amount: 'damage', target: 'target' }, { kind: 'statmod', mods: { spellAmpPct: 18, castRange: 120 }, duration: 'duration', target: 'self' }], vfx: vfx('global-mark', '#7ad85a', '#d8ffd0') }
    ]
  }),
  hero({
  ...
  hero({
    ...
    abilities: [
      ...
      { id: 'techies-proximity-mines', name: 'Proximity Mines', targeting: 'ground-aoe', ult: true, castRange: 900, castPoint: 0.45, manaCost: [110, 140, 170], cooldown: [28, 22, 16], values: { damage: [280, 420, 560], radius: [360, 390, 420], duration: [20, 24, 28] }, effects: [{ kind: 'exotic', id: 'remote-mines', params: { heroId: 'techies' } }, { kind: 'zone', at: 'point', zone: { shape: 'circle', radius: 'radius', duration: 'duration', onEnter: { affects: 'enemies', windowSec: 2, effects: [{ kind: 'damage', dtype: 'magical', amount: 'damage', target: 'target' }, { kind: 'status', status: 'stun', duration: 0.5, target: 'target' }] } } }], vfx: vfx('ground-aoe', '#ff8a2f', '#f0f060') }
    ]
  })
];
