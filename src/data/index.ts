...
import { buildQuestGivers } from './quests/givers';
...
export function registerAllContent(): void {
  ...
  for (const g of buildQuestGivers(ALL_REGIONS)) REG.registerQuestGiver(g);
  ...
  for (const d of ALL_DISHES) REG.registerDish(d);
