...
- **Expand likeness overlays. [DONE]** `applyHeroLikeness` (`models.ts`) overshot the "9 → 65" target: `HERO_LIKENESS_PROFILES` now carries ~100 entries (starters, the rest of the Phase 1 six, gym leaders, Elite Five, and the broad roster), each evoking the Dota silhouette, gear, weapon, and colors off the `silhouette`/`palette` data already on every hero. The bar is "named on sight," not a copy.
...
**Status (2026-06-14): all phases (0–5) shipped.** Render-side async loaders live in `asset-loaders.ts` (`loadTex`/`loadHdr`/`loadModel` + meshopt decoder + `SkeletonUtils`-safe `cloneModel`); every loader is browser-guarded and resolves null so the procedural floor is always the fallback. Phases 0–3 shipped 2026-06-13 (terrain PBR + HDRI, instanced foliage/props/town, creep GLBs). Since then the keystone and surface phases landed and the markers below are reconciled to the code: Phase 3.5 (skeletal-animation bridge) is live — `AnimationMixer` + an authored-clip path in `animator.ts`; Phase 4 (VFX + audio) shipped — archetype-shaped telegraph decals, element-tinted projectiles/trails, reaction bursts, and the full procedural-floor-plus-CC0 audio surface (music beds, reverb send, sampled cast layer — see `SOUND_MAP.md`); Phase 5 (heroes) shipped — `ENABLED_HERO_MODELS` is populated (80 humanoid GLBs + 31 shared creature bases, 11 procedural holdouts), and `applyHeroLikeness` overshot its "9 → 65" target to ~100 likeness profiles. Locked calls held: hero GLBs as a few **CC0 archetype bases retextured across many heroes**; audio as a **procedural floor plus a small CC0 sample layer**; VFX sprites **hybrid** (generated cheap + Kenney CC0 for heavy impacts).
...
- **Phase 2 — Foliage, rocks, props, town. [DONE]** Quaternius trees/rocks instanced over the same deterministic placements (`swapToInstancedModels` + `instancedFromModel`); the box-town swaps to Medieval Village buildings (`swapTownBuildings`). Instanced primitives / box huts stay live until GLBs load. Billboard grass tufts shipped 2026-06-14: `planGrassTufts` (pure, deterministic, biome-weighted, clearing-aware) feeds `buildGrassTufts` (instanced alpha-tested cross-quads), tier-gated by the new `QualityPreset.grassDensity` (low 0 / medium 0.5 / high & ultra 1) so the no-tuft floor on low tier is exact; covered by `src/test/terrain-foliage.test.ts`.
...
- **Phase 3.5 — Skeletal animation bridge (the keystone). [DONE]** Authored clips now play, driven off the sim timing the animator already reads. `UnitRig` carries an optional `mixer: THREE.AnimationMixer` + an `actions` clip map; `mountHeroModel` builds the mixer and maps clips (idle/run/attack/cast/channel/death) when the GLB carries them, and `animator.ts` runs a clip-driven path (`switchAuthoredAction` crossfades idle↔run by measured speed; attack/cast/channel/death fire on `windupUntil`/`castingUntil`/`channel`/`alive`, honoring the §3.0 timing contract), with the procedural math staying the fallback for primitive rigs (selected by whether `rig.mixer` exists). This instantly animates the shipped creep GLBs and unblocked Phase 5.
...
- **Phase 4 — VFX + audio surface. [DONE]** Two independent tracks, both shipped. *VFX (§7):* `vfx.ts` carries an archetype-shaped telegraph-decal family (ring/spiked/hatched/dotted selected by `telegraphKindFor`), element-tinted projectiles with ribbon trails (`TRAIL_LEN` pooled trail meshes), shaped element particles (ember/snow/shard), and dedicated reaction bursts (`reactionPalette`/`reactionSprite` for Vaporize/Melt/Overload), with a Kenney CC0 atlas layered over the generated sprites where present and the whole pool capped to the transient budget. *Audio (§3.12):* the procedural floor plus a small CC0 sample layer — sampled cast voices, per-biome composed music beds with combat/night ducking on a real `music` channel, a `ConvolverNode` reverb send, and event cues — documented in full in `SOUND_MAP.md` and enforced by `src/test/audio.test.ts`. Decisions locked 2026-06-13 held: VFX sprites **hybrid**; audio **procedural floor + small CC0 sample layer**.
