...
This document is now a record of work done, not a forward plan. The §0 findings (Captain's Call was not control, the gambit AI was shallow, raids were not played) have all been addressed, and the C4 readability layer that ties them together has shipped. All five slices (C0–C4) are done.
...
| C4 — readability & feedback | **Shipped** | `Game.combatReadout()` is the headless, determinism-safe read over the active sim that the HUD turns into a readability layer (`src/ui/hud.ts` `renderCombatReadout`, `#combat-readout`): cast bars on ally/enemy casts (enemy big ults flagged and sorted first), a boss aggro/threat marker that shows the current target and flips on taunt, the team shared-focus indicator (from `Sim.teamMind`), and the "ult ready → seize" prompt. The windup/anticipation/telegraph contract remains in `PRESENTATION_SPEC.md`. Covered by `src/test/combat-readout.test.ts`; `boundary.test.ts` green. |
...
| C4 | ✅ Done | `Game.combatReadout()` produces the readability snapshot over the active sim — cast bars (enemy big ults flagged/sorted first), the boss aggro/threat marker (current target + taunt flip), the shared-focus indicator (`Sim.teamMind`), and the ult-ready seize list — and `renderCombatReadout` (`#combat-readout`) draws it, with the full overlay during a live raid/gym. The read never changes a combat result, so the determinism tests stay green; `src/test/combat-readout.test.ts` covers the snapshot in the overworld and a live raid, and `boundary.test.ts` stays green. |
