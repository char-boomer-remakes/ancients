...
The overhaul has landed in full — all eight slices (G0–G7) are shipped or held. The Genshin-by-systems spine (Approach A, §2) held: the overworld now reads as traverse → discover → solve → fight → bank, not just `fight camp → walk → fight camp`.

| Slice | State | Notes |
|-------|-------|-------|
| G0 — timing contract | **Held** (standing gate) | The sim owns timing; the renderer animates it. Dash respects the windup/backswing rules and is root-stopped. The "dash mid-windup deals no damage" assertion now exists: a dash issued during an attack windup pushes a forced move that zeroes `windupUntil`, so the strike is cancelled and lands no damage (`src/test/gameplay-overhaul.test.ts`). |
...
| G3 — verticality & traversal | **Shipped** | `updateTraversal` runs the climb/glide/swim locomotion states: a climb is a scripted, stamina-draining ascent between elevation tiers (running dry mid-climb slides back down); a glide is a free, committing descent; entering a `waterZone` slows movement and drains stamina, and deep water at zero stamina washes back to dry land (a soft fail). `tryClimb`/`tryGlide` work off the interact key; the scene lifts the active hero by `Unit.renderHeight`; the HUD shows the locomotion state and a connector prompt. Icewrack is authored with three tiers, two climb points, a glide line, and a deep frozen lake. `src/test/traversal.test.ts` + data-lint. |
...
| G7 — support & ambience | **Shipped** | Cooking: `DishDef` + `Game.cookDish` at a town/shrine spends a gold/ingredient cost for an out-of-combat party heal, a one-shot revive of a fallen hero, or a timed exploration buff (the buff rides the existing `buff`-status statmod path); surfaced in the Field Kitchen under Town Services. Elemental weather: `RegionDef.weather` applies an element to outdoor units through the §3.3/§3.4 field path (`updateWeather` → `applyElementAura`), gated by Resonance and the day/night clock — Icewrack gets a night-frost Cryo. `src/test/cooking-weather.test.ts` + data-lint. (Cosmetic particle weather from `GRAPHICS_SPEC §5.6` is unchanged and separate.) |
...
|-------|--------|-----------|
| G0 | ✅ Done (gate held) | The boundary test stays green (timing in core, animation in engine); every dodge-check attack added in later slices has a visible windup ≥ its fair-reaction window; dash cancels backswing but never windup-into-free-damage. The remaining assertion now exists: a dash issued mid-windup pushes a forced move that zeroes `windupUntil`, so the cancelled strike lands no damage (`src/test/gameplay-overhaul.test.ts`). |
...
| G3 | ✅ Done | `updateTraversal` runs the climb/glide/swim states off the elevation graph: a climb is a scripted stamina-draining ascent (dry mid-climb → slide back), a glide is a free committing descent, and a `waterZone` slows + drains, with a deep-water zero-stamina washback to dry land. `tryClimb`/`tryGlide` bind to the interact key, the scene lifts the hero by `Unit.renderHeight`, and the HUD shows the state + a connector prompt. Icewrack is authored to use all three; `src/test/traversal.test.ts` + connector data-lint; `boundary.test.ts` stays green. |
...
| G7 | ✅ Done | `Game.cookDish` at a town/shrine spends a gold/ingredient cost for an out-of-combat party heal, a one-shot revive of a fallen hero, or a timed exploration buff carried on the existing `buff`-status statmod path (Field Kitchen UI). `RegionDef.weather` applies an element to outdoor units through the §3.3/§3.4 field path (`updateWeather` → `applyElementAura`), gated by Resonance and the day/night clock (Icewrack night-frost Cryo). `src/test/cooking-weather.test.ts` + data-lint. Cosmetic particle weather (`GRAPHICS_SPEC §5.6`) is separate and unchanged. |
