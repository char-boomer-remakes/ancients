...
| Env/UI assets | terrain PBR (ambientCG), 2 HDRIs (Poly Haven) **both wired** — day + night beds swap by the cycle (`scene.applyEnvPhase`); original tiling water normal (`textures/water/water_normal.webp`) layered into the water shader; OFL display font (Cinzel) vendored + wired via `@font-face`; original carved-frame/parchment/gem/portrait SVG UI frames generated by `generate_ui_assets.mjs` and used by HUD panels, slots, portraits, minimap, modals, and cinematics; foliage + town (Quaternius) |
...
| Audio | synth floor (`SoundArchetype` ×12, including `lightning`) **+ sampled enhancement layer**: `engine/sampled-audio.ts` decodes original generated per-biome music beds, original one-shot WAVs, and curated Kenney CC0 SFX variants (`/assets/audio/*`), layered over the synth on medium+; synth stays the guaranteed fallback (boot floor / headless / missing file) |
| Pipeline | `build_assets.mjs` (resample/prune/dedup/meshopt/webp + **palette recolor: flat factor _and_ A4 `tritone` texture-space gradient map** + audio/font/vfx/holdout groups), generators (`generate_vfx_atlas`/`generate_water_normal`/`generate_audio`/`generate_holdout_signatures`), curated CC0 audio staging under `tmp/asset_src`, `assets.ts` + `sampled-audio.ts` loaders + fallback, `ASSETS.md` ledger, `manifest.json` |
...
- **A7 — generated animated holdout replacement GLBs. SHIPPED.** The 11 abstract
  holdouts now also ship original hierarchy-animated replacements under
  `/assets/holdouts/replacements/<id>.glb`. Each file has named
  `idle/run/attack/cast/channel/death` clips, so `scene.ts` can mount it through
  the normal `heroAssetEntry → HeroAssetLoader.loadHero → mountHeroModel` path and
  let `animateAuthoredRig` drive it from sim state. If a replacement is missing or
  fails, `scene.ts` keeps the animated procedural rig and mounts the older A6
  additive signature kit instead. This is a generated part-rig replacement pass,
  not an external skeletal art pack.
...
- **Music beds (done).** One 44.1 kHz stereo, seamlessly-looping ambient bed per
  biome (`generate_audio.mjs`, original). `ProceduralAudio.update` crossfades
  the composed bed for the current biome, ducks it in combat/at night, and swaps
  beds on a biome change; with no decoded file the game stays SFX-only. The bed
  has its own `music` volume channel/slider; the procedural synth *drone* stays
  off by default (it read as a constant hum). Gated to medium+ via `Game` →
  `enableSampledAudio`.
- **Sampled SFX (done).** `crit`, `impact-heavy`, `fanfare`, `whoosh`,
  `projectile-hit`, `blade-draw`, `coin`, and one `cast-*` cue for every
  `SoundArchetype` layer over the synth on crits, big physical hits,
  celebratory stingers, big-shape spell casts, ranged/miss whooshes,
  projectile arrivals, blade casts, pickups, and all hero/spell casts;
  pooled/throttled by the existing voice logic. The cast map is documented in
  `docs/design/SOUND_MAP.md`.
- **Curated CC0 variants (done).** A small Kenney CC0 subset from RPG Audio,
  Impact Sounds, Music Jingles, and Digital Audio ships under
...
- **UI frame assets (done).** Original SVGs under `ui/frames/` provide a carved
  brass frame, parchment panel, gem slot, and portrait frame. `ui/styles.css`
  layers them over existing CSS gradients for HUD panels, party cards, modals,
  cinematic cards, ability/item slots, portraits, minimap, and toasts; missing
  files fall back to the previous CSS-only treatment.
...
16. **UI frame asset pass** — carved-frame/parchment/gem/portrait SVGs.
    **Shipped.** `generate_ui_assets.mjs` writes the original frame textures and
    `ui/styles.css` applies them to the HUD shell while retaining CSS fallbacks.
17. **A7** — generated animated holdout replacement GLBs. **Shipped.** The 11
    abstract heroes now resolve `/assets/holdouts/replacements/<id>.glb` through
    `heroAssetEntry`; those files carry `idle/run/attack/cast/channel/death` clips.
    If a replacement misses, the A6 additive signature fallback still mounts over
    the procedural rig.
...
> enhanced. The 11 abstract holdouts now prefer generated animated replacement
> GLBs, with A6 additive signatures retained only as fallback.
...
> stops, generated detail/normal passes), generated boot motion trails, or richer
> skeletal/skin-weighted upgrades over the generated holdout replacements.
...
- **Static holdout GLBs could regress animation.** Resolved in two layers: A7
  replacement GLBs carry named animation clips and mount through `mountHeroModel`;
  missing/failing replacements fall back to A6 additive signature kits mounted over
  the animated procedural rig.
