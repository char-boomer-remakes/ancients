...
The layer the game has never had: **plot that lands, lore that deepens, and cinematic cut-scenes for the moments that matter** — recruiting a hero, breaking a boss, opening a raid, holding a legendary, crowning a champion. Companion to `SPEC.md` (design target), `PRESENTATION_SPEC.md` (look/feel/juice), `DECISIONS.md` (calls made), and `PROGRESS.md` (status). Same crunch-mode rules as `SPEC.md §0`: this is direction and priority, not a gate.

The game is content-complete through Phase 5 and now has a juice layer planned in `PRESENTATION_SPEC.md`. What it still lacks is **story weight**. A player can cross ten regions, bind a hundred heroes, and crown themselves at the Tower of the Ancients without the world ever telling them *why any of it matters*. The lore exists — it lives in scattered `lore` strings on regions, heroes, and items, and in one-line `dialogue[]` arrays on raids and gyms — but it never assembles into a plot, and the biggest beats pass with a toast and a stinger. No game is complete without the moment the music drops, the bars close in, and the world shows you what you just did.
...
An honest read of the narrative layer, with file references so the work has a starting line.

**Lore is real but inert.** The world has a strong, consistent voice. The title carries the whole premise in one line (`src/ui/title.ts`): *"The Mad Moon broke. Its shards remember every war."* Every region has a `lore` paragraph (`src/data/regions/*`), every hero a `lore` + `title` + `barks[]` (`src/core/types.ts`, `HeroDef`), every item a punchy micro-fiction (`src/data/items/index.ts` — *"A victory condition with a handle. It drops when pride dies."*). But none of it is **sequenced**. There is no prologue, no inciting event, no escalation, no resolution — just a pile of well-written flavor the player reads in menus if they go looking.

**Dialogue exists only as flat string arrays.** Raids carry `dialogue: string[]` (`src/data/raids.ts`), gyms and trials and the Elite Five carry the same (`src/data/gyms/index.ts`, `src/data/quests/index.ts`, `src/data/drafts.ts`). These are shown as HUD `msg()` toasts or pre-fight prompts — text on a card, no staging, no camera, no scoring of the moment. The Champion's three lines (`src/data/drafts.ts`, `championDialogue`) are the closest thing the game has to a climax, and they render as plain toasts.

**The big beats pass in silence (narratively).** `PRESENTATION_SPEC.md §1` is making *rewards* land — gold, loot, crits. But the **story** beats — your first bind, an eighth badge, the moment a marquee raid boss turns to face you, holding the Aegis for the first time, beating the Twice-Crowned — get the same generic toast-and-stinger as picking up 4 gold. The narrative peaks and the mechanical peaks are not the same peaks, and right now neither is dramatized as story.

**No cut-scene machinery of any kind.** Confirmed against `src/engine/` and `src/systems/`: there is no timeline sequencer, no camera-spline or `cinematic` camera mode (only `gameplay` / `map` exist, `scene.ts`), no letterbox or dialogue-card overlay (HUD has `msg()` toasts and modals only, `src/ui/hud.ts`), no dialogue tree, no scripted NPC blocking, no cut-scene replay gallery. Voice is synthesized stingers and effort-blips only (`src/engine/audio.ts`) — no narration.

**But the foundations are unusually good.** The `SimEvent` bus (`src/core/types.ts`, the `SimEvent` union — `cast`, `death`, `bark`, `revive`, `levelup`, `summon`…) already decouples logic from presentation. The animator plays poses from a gesture vocabulary with no keyframes (`src/engine/animator.ts`). VFX has ~12 data-parameterized archetypes (`src/engine/vfx.ts`). Audio has a stinger system (`capture`, `badge`, `raid-clear`). The codex/journal already gate narrative content on encounter (`Game.codexEntries()`, `Game.journalSections()`). And `PRESENTATION_SPEC.md §4.6` is building a **Moment Director** that composes camera, sound, VFX, and time-feel from an intensity level — the cut-scene system is that director's narrative sibling, and should share its impact budget and its accessibility caps.

**Two highest-leverage moves.** First, **assemble the existing lore into a plot** (`§2`) — a prologue, an escalating spine across the eight badges, and a climax at the Tower — so the world the player crosses has an arc, not just atmosphere. Second, build **one cut-scene system** (`§4-5`) that every key event routes through, then author the **bespoke set-pieces** (`§6`) for the dozen moments that carry the story. Everything else is templated stingers reusing the same machinery.
...
- **Seen → auto-fast-forward.** A cut-scene the player has already watched once (tracked in `GameSave.journalSeen`) defaults to auto-skip on its next trigger, surfacing only as the equivalent toast/bark so the information still lands. Replaying at full length is opt-in from the gallery (`§8`).
...
| | The Sundered Betrayer *(planned, `addendum §2.1`)* | Illidan | `terrorblade` / `night-stalker` | Fel-green metamorphosis, the betrayer's brand — authored when the boss ships; not yet in `raids.ts` |
