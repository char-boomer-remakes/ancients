...
Phase 4's **animation gesture player** (drives every ability/item active from `anim`/`animProfile`, replacing hardcoded `heroId` branches) closed in P6 M6; **barks from the sim core** (cast + kill) closed in P6 M7. In-character dialogue depth for trials/gyms/Elite/raids remains → P6 M8.
...
| 4c | Barks fire from the sim on cast/kill/recruit/low-HP/badge, rate-limited; in-character original dialogue for trials/gyms/Elite/raids | PARTIAL — M7 (cast + kill barks from sim core, rate-limited; recruit-join surfaced; dialogue depth → M8) |
...
| G2 | All §6 tests pass; `npm test` + `npm run build` green | IN PROGRESS (M1–M7 green; 742 tests) |
