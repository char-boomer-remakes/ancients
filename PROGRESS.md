...
- 2026-06-13: Gameplay 2.0 closeout (the last open items of `GAMEPLAY_2.0_REHAUL.md`). §5.4 — centralized the ~40 inline AI scorer constants (danger biases `casterBias`/`heroBias`/`lowHpPenalty`, cluster radius, zone-escape margin, kite step/defaults, boss-posture score multipliers, hand-tuned + intent-fallback item score weights, item leash/heal radii, combo defaults, ult-hold slope/floor, and the depth-scaled raid radii + boss phase-FSM constants) into a structured `TUNING.ai` sub-block, so a balance pass is a data edit; values are unchanged so every existing test stays green. §5.7 — extended ai-depth past the normal-tier baseline to *also* dial the new behaviors: `manaAdjustedScore` and `comboAdjustedScore` now sharpen with `aiDepthBonus(u) × depthDisciplineGain` (Hell-tier enemies husband mana and chain combos harder), on top of the already depth-gated AoE-ult hold. §3.3 — broadened boss theming so every boss is an efficient themed home: the str/agi/int classification now covers the full boss roster (no str/agi boss silently vends caster cores), and crit/attack carries gained a Daedalus/MKB/Mjollnir pool (the doc's "a Daedalus farms from a crit carry"). Added `src/test/loot-sources.test.ts` as the verifiable §3.3 contract — every Immortal/Arcana catalog item resolves to at least one boss/raid/dungeon home (scripted Roshan repeat drops allowlisted). §5.6 — confirmed `most-clustered`, `bestCluster`, and the boss cluster target all run their radius scans through the spatial grid (`unitsInRadius`/`forEachNearbyUnit`), unified on `TUNING.ai.clusterRadius`. §9 — the loot-pacing + combat-scaling sim contracts (the doc's authority) pass within their per-band floor/ceiling bands, so the constants need no retune. `npm run typecheck` + full `npm test` (45 files, 1331 tests) + `npm run build` green; edited-file lints clean. `GAMEPLAY_2.0_REHAUL.md` is now fully implemented across all four pillars.
